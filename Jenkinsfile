	...
	agent {
		...
		stages{
			stage ("Pull the code from SCM"){
				steps {
					git branch: 'main', url: 'https://github.com/jaatbreak/java-programm.git'
					}
				}
			...
			stage("Production  Env") {
				...
				steps {
					sshagent(['kuber_node']) {
			    	 	sh "ssh -o StrictHostKeyChecking=no ubuntu@13.212.147.47 sudo kubectl run c1 amansingh12/java-app:$BUILD_TAG'
